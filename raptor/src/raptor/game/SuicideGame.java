...
public class SuicideGame extends Game {
	...
	public PriorityMoveList getLegalMoves() {
		...
		LOG.debug("Possible Moves = " + java.util.Arrays.toString(result.asArray()));
		...
		for (int i = 0; i < result.getHighPrioritySize(); i++) {
			...
			if (move.isCapture()) { containsCaptures = true; }
			}
		...
		for (int i = 0; i < result.getLowPrioritySize(); i++) {
			if (containsCaptures) { result.removeLowPriority(i); i--; continue; } // remove all, since there are no non-capture legal moves if can capture
			
			Move move = result.getLowPriority(i);
			forceMove(move);
			
			if (!isLegalPosition()/* || move.isCastling()*/) { // castling not allowed in suicide
